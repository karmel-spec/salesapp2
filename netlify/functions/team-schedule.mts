/**
 * Team Schedule — two-way sync API for the Shop Manager's Schedules view.
 * Renders/edits the "Team Schedule" tab on the Friday-report spreadsheet
 * (Name | Role | Mon…Sat | Notes) through the sales console's service
 * account. Weekly scheduling drafts read this tab as the source of truth
 * for shift hours.
 *
 *   GET  ?key=…             → {tabs: {"Team Schedule": string[][]}, fetchedAt}
...
async function sheets(path: string, init?: RequestInit): Promise<any> {
  ...
  if (!res.ok) {
    ...
    const hint = res.status === 403
      ? " — share the report sheet with the sales service account as Editor"
      : "";
...
export default async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const ranges = TABS.map((t) => `ranges=${encodeURIComponent(`'${t}'!A1:M200`)}`).join("&");

...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // plus the Customer Service queue (Support status). Inactive, lost, won,
    // closed and unqualified are never counted here.
    if (req.nextUrl.searchParams.get("count") === "1") {
      ...
      const support = leads.filter((l) => l.statusBucket === "support").length;
      return NextResponse.json({ brighamActive, othersActive: open.length - brighamActive, support });
    }

...
export const BOARD_PEOPLE: BoardPerson[] = [
  ...
  { key: "alisa", name: "Alisa", role: "marketing", mailbox: "alisa@brighamlarsonpianos.com", taskOwner: "Alisa Miller", rep: "Alisa" },
  { key: "lisa", name: "Lisa", role: "admin", mailbox: "lisa@brighamlarsonpianos.com", taskOwner: "Lisa Litton", rep: "Lisa" },
  ...
  const board: Board = {
    ...
    totals: {
      ...
      people: rows.map((r) => ({
        ...
        cards: r.tasks ? r.tasks.new : BOARD_PEOPLE.find((p) => p.key === r.key)?.taskOwner ? 0 : null,
        askBrigham: r.tasks ? r.tasks.askBrigham ?? 0 : BOARD_PEOPLE.find((p) => p.key === r.key)?.taskOwner ? 0 : null,
      })),
    },
  };
